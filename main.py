...
def track_price():
    ...
    # Scraping Data
    def parse_single_item(html_content):
        # Soup Object
        ...
        product_name_details_text = product_name_details.text if product_name_details else "Title Not Found (Dead Link)"

        all_products_data.append({"Product Name": product_name_details_text, "Product Price": product_price_text})
    ...
    def parse_search_results(html_content):
        ...
        for product in all_products:
            ...
            product_name_details_text = product_name_details.text if product_name_details else ("Title Not Found (Dead "
                                                                                                "Link)⚠️")
            product_price_text = product_price.text if product_price else "Out of Stock/No Price🛒❌"
    ...
    def route_and_parse(current_url, html_content):
        ...
        if "/dp/" in current_url or "/gp/" in current_url:
            parse_single_item(html_content)
        elif "/s?" in current_url or "/search" in current_url:
            parse_search_results(html_content)
    ...
    with sync_playwright() as p:
        ...
        if not chrome_path:
            results_txt_box.insert("end", "❌ Chrome not found on this system. Please install Google Chrome.\n")
        ...
        for i, url in enumerate(urls, start=1):
            ...
            try:
                ...
                results_txt_box.insert(f"Failed to fetch {url}:{e}")
    ...
    if save_path:
        with open(save_path, "w", encoding="utf-8", newline="") as new_csv_file:
            field_names = ["Product Name", "Product Price"]
...
root.title("Amazon-Price-Name | Scraper")
...
# Label-Header
l_head = customtkinter.CTkLabel(root, text="Amazon Data Scraper", font=("Press Start 2P", 16), text_color="green")
...
# Buttons
track_product_price_btn = customtkinter.CTkButton(f_btn_label_txt_box, text="Scrape Data", font=("Helvetica", 12),
